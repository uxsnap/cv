<template>
  <div id="vending" class="vending" ref="vending">
    <img
      ref="vendingImage"
      class="vending__image"
      src="~/assets/images/vending.svg"
    />
    ...
    <div
      @click="getItemFromVending"
      class="vending__lever"
...
export default {
  ...
  methods: {
    ...
    resizeWalls() {
      this.walls = createWalls(this.$refs.vendingContainer);
    ...
    addBalls(propBalls) {
      ...
      for (let i = 0; i < propBalls.length; i++) {
        this.dataBalls.push(
          Bodies.rectangle(randomRange(50, 200), randomRange(10, 200), 30, 30, {
            render: {
              sprite: {
  ...
  watch: {
    ...
    lastDeleted(newVal, oldVal) {
      ...
      World.remove(
        this.world,
        this.dataBalls.find((item) => item.id === newVal)
      );
    },
  },
  mounted() {
